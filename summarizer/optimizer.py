...
#         bInd = self._binarize(ind)
    ...
    def getBetter(self, ind1, ind2, bInd1, bInd2):
        ...
        '''
        ind1_size = self.getNumOfTerms(bInd1)
        ind2_size = self.getNumOfTerms(bInd2)
...
#             self.hof.update(self.pop)
        ...
        print self.getNumOfTerms(self.best_so_far)
        # get best of gen
#         self.best_gen.append(self.hof[0])
            
        record = self.stats.compile(self.pop) if self.stats else {}
        self.logbook.record(gen=0, evals=len(self.pop), size=self.getNumOfTerms(self.best_so_far_b), **record)
...
#                 self.hof.update(newpop)
            ...
            self.logbook.record(gen=gen, evals=len(newpop), size=self.getNumOfTerms(self.best_so_far_b), **record)
            print self.logbook.stream
...
#         print 'size: ', self.getNumOfTerms(self.best_so_far_b)
#         print solution
        return solution
